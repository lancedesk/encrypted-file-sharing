    ...
    public function efs_create_private_folder() 
    {
        ...
        if (!file_exists($private_dir)) 
        {
            ...
            if (mkdir($private_dir, 0755, true)) 
            {
                ...
                $efs_local_file_handler->log_message($log_file, 'Private uploads folder created at: ' . realpath($private_dir));
            } 
            else 
            {
                /* Log failure */
                $efs_local_file_handler->log_message($log_file, 'Failed to create private uploads folder at: ' . $private_dir);
        ...
        else 
        {
            ...
            $efs_local_file_handler->log_message($log_file, 'Private uploads folder already exists at: ' . realpath($private_dir));
        }
    }
}